...
export function AdminMajorsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setErrorMessage(getApiErrorMessage(error, t('admin.majors.error.load')))
      }
    }
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!Number.isInteger(parsedFacultyId) || parsedFacultyId <= 0) {
      setErrorMessage(t('admin.majors.error.facultyRequired'))
      return
    }
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, t('admin.majors.error.save')))
    }
  }
    ...
    <section className="space-y-4 rounded-card border border-primary/10 bg-surface p-4 shadow-soft">
      ...
      <form className="grid gap-3 md:grid-cols-5" onSubmit={onSubmit}>
        <input
          ...
          placeholder={t('admin.majors.placeholder.name')}
          value={name}
        ...
        <input
          className="rounded-card border border-primary/20 px-3 py-2 text-sm"
          placeholder={t('admin.majors.placeholder.slug')}
          value={slug}
        ...
        <select
          ...
          required
        >
          <option value="">{t('admin.majors.selectFaculty')}</option>
        ...
        <label className="flex items-center gap-2 rounded-card border border-primary/20 px-3 py-2 text-sm text-textSecondary">
          ...
          {t('admin.majors.active')}
        </label>
        <div className="flex gap-2">
          <Button variant="primary" type="submit">
            {selectedMajor ? t('admin.majors.update') : t('admin.majors.create')}
          </Button>
          {selectedMajor ? (
            <Button
              type="button"
        ...
        <table className="min-w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-primary/15 text-left text-textSecondary">
               <th className="py-2 pr-3">{t('admin.majors.column.id')}</th>
               <th className="py-2 pr-3">{t('admin.majors.column.name')}</th>
               <th className="py-2 pr-3">{t('admin.majors.column.facultyId')}</th>
               <th className="py-2 pr-3">{t('admin.majors.column.active')}</th>
               <th className="py-2 pr-3">{t('admin.majors.column.slug')}</th>
               <th className="py-2">{t('admin.majors.column.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {majors.map((major) => (
              <tr key={major.id} className="border-b border-primary/10">
                ...
                <td className="py-2 pr-3">{major.is_active ? t('common.yes') : t('common.no')}</td>
