      ...
      <details className={styles.filtersDisclosure}>
        ...
        <div className={styles.filtersPanel}>
          <details className={`${styles.control} ${styles.comparisonControl}`}>
            <summary><span className={styles.controlLabel}>Comparar con</span><span className={styles.comparisonValue}>{comparisonSummary}</span><span className={styles.chevron} aria-hidden="true">⌄</span></summary>
            <div className={styles.comparisonMenu} role="listbox" aria-label="Procesos para comparar" aria-multiselectable="true">
              {comparisonOptions.map((process) => {
                ...
                return <label className={styles.comparisonOption} key={process.id}><input type="checkbox" value={value} checked={checked} disabled={disabled} onChange={(event) => onChange(primaryId, event.target.checked ? [...comparisons, value].slice(0, 3) : comparisons.filter((id) => id !== value))} /><span>{process.name}</span></label>;
              })}
